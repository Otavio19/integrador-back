...
class OrderProduct {
  async list() {
    const products = await sql`SELECT * FROM order_product`;
    return products;
  }

  async getOrdersByCompany(id_company) {
    const orders = await sql`
      SELECT users.name as seller, client.name as client, orders.price, orders.id, orders.source, orders.status, orders.created_at
      FROM orders
  ...
  async getAllByOrder(id) {
    ...
    try {
      const orderInfo = await sql`
        SELECT users.name, client.name, orders.id, orders.price, orders.id_client
        FROM orders
        INNER JOIN users ON users.id = orders.id_seller
        INNER JOIN client ON client.id = orders.id_client
        WHERE orders.id = ${id}`;
      ...
      const products = await sql`
        SELECT order_product.price, order_product.amount, product.name
        FROM order_product
  ...
  async addOrder(orderId, info) {
    ...
    const response = await sql`
      INSERT INTO orders (id, id_seller, id_company, id_client, price, source, status) 
  ...
  async addProduct(product) {
    ...
    const response = await sql`
      INSERT INTO order_product (id, id_order, id_product, price, amount)
